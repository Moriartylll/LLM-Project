...
# ---------------- Config ----------------
CHAT_MODEL = "Qwen/Qwen2.5-7B-Instruct"   # small & quick
...
# Init chat model (prefer 4-bit on consumer GPUs)
tok = AutoTokenizer.from_pretrained(CHAT_MODEL, use_fast=True)
...
ORG_RE = re.compile(r"\b(Org\s*nr|Organisationsnummer)\b[:\s]*([A-Z]{2}\d{6,}|\d{6,})", re.I)
TIME_RE = re.compile(r"\b(Tid)\b[:\s]*([0-2]?\d[:.][0-5]\d)", re.I)
ZIP_CITY_RE = re.compile(r"\b(?P<zip>\d{3}\s?\d{2})\s+(?P<city>[A-Za-zÅÄÖåäö\-]+)", re.U)
STREET_RE = re.compile(r"([A-Za-zÅÄÖåäöÉéÜüÖöÄä\- ]+)\s+\d+[A-Za-z]?$", re.U)
SHORTNAME_RE = re.compile(r"^[A-ZÅÄÖ]{3,20}$", re.U)
...
def first_match(pattern: re.Pattern, seq: List[str], limit: Optional[int] = None, start: int = 0):
    rng = seq[start:(start+limit) if limit else None]
    for i, L in enumerate(rng, start):
        m = pattern.search(L)
        if m:
            return i, m
    return None, None
...
class LineItem(BaseModel):
    ...
    discount_amount_total: Optional[float] = None
    unit_price_after_discount: Optional[float] = None
    line_total_after_discount: Optional[float] = None
...
class Receipt(BaseModel):
    ...
    store_short_name: Optional[str] = None
...
def parse_lines_to_receipt(lines: List[str], file_name: str, source_type: str) -> Receipt:
    ...
    # ---------- HEADER FIELDS ----------
    # store_name: line that has vendor + more words (e.g., "ICA Supermarket Medis")
    store_name = None
    if vendor:
        for L in lines[:50]:
            if re.search(vendor, L, re.I):
                if len(L.strip().split()) >= 2:
                    store_name = L.strip()
                    break

    # store_short_name: a SHOUTY token near top (avoid matching vendor token "ICA")
    store_short_name = None
    for L in lines[:60]:
        T = L.strip()
        if SHORTNAME_RE.match(T) and not re.search(r"\d", T):
            if vendor and re.fullmatch(vendor, T, re.I):
                continue
            store_short_name = T
            break

    # org number and time
    _, morg = first_match(ORG_RE, lines, limit=120)
    org_number = morg.group(2) if morg else None

    _, mtime = first_match(TIME_RE, lines, limit=120)
    tval = mtime.group(2) if mtime else None
    if tval and "." in tval:
        tval = tval.replace(".", ":")
    ...
    zip_code = None
    city = None
    street_idx, mstreet = first_match(STREET_RE, lines, limit=50)
    if mstreet:
        address = mstreet.group(0).strip()
    zidx, mzip = first_match(ZIP_CITY_RE, lines, limit=60)
    if mzip:
        zip_code = mzip.group("zip").replace(" ", "")
        city = mzip.group("city").title()
    ...
    for L in lines:
        ...
        if m2 and not SKIP_ROW_RE.search(Ls):
            ...
            if INCLUDE_DISCOUNTS and lsum is not None and lsum < 0 and items:
                ...
                if target_idx is not None:
                    ...
                    if bundle_match:
                        ...
                        if n and bundle_price is not None and n > 0:
            ...
            # Not a discount → ignore (we only keep products; fallback non-negative lines usually are headers/footers)
...
def build_context() -> str:
    ...
    for r in RECEIPTS.values():
        rows.append({
            ...
            "store_short_name": r.store_short_name,
